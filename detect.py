...
def factory(idx):
    def qwen_forward(self, x):
        ...
        activated_value[idx, :] += activation.sum(dim=(0,1))
...
activated_value = torch.zeros(num_layers, intermediate_size, dtype=torch.float32).to('cuda')
...
n = len(inputs["input_ids"][0]) * total_len
...
torch.save(activated_output, f'data/activation.train.value.{model_suffix}.base')
...
for key, items in instructions.items():
    ...
    activated_output = dict(n = n, activated_value=activated_value.to('cpu'))
    torch.save(activated_output, f'data/activation.train.value.{model_suffix}.{key}.A')  
    ...
    activated_output = dict(n = n, activated_value=activated_value.to('cpu'))
    torch.save(activated_output, f'data/activation.train.value.{model_suffix}.{key}.B')
