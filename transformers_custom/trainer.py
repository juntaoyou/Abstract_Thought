...
class Trainer:
    ...
    def mask_gradients(self, model, activate_neuron):
        ...
        Only the neurons specified in activate_neuron are allowed to update.
        
        Args:
            model: The model with parameters and gradients.
            activate_neuron (dict): Dict of layer → index sets for each projection type.
        """
        if activate_neuron is None:
            return
        ...
        for name, param in model.named_parameters():
            # Skip if no gradient or gradient is invalid
    ...
    def training_step(
        ...
        if self.use_apex:
            ...
            self.mask_gradients(model, activate_neuron)
