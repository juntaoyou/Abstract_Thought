...
class LlamaForCausalLMDetect(GenerationMixinCustom, LlamaPreTrainedModel):
    ...
    def forward(
        ...
        top_ratio_atten: float = 0,
        top_ratio_ffn: float = 0,
        activate_keys_fwd_up_set: Optional[dict] = None,
        activate_keys_fwd_down_set: Optional[dict] = None,
        activate_keys_q_set: Optional[dict] = None,
        activate_keys_k_set: Optional[dict] = None,
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if early_exit_layers is not None:
            ...
            for i, early_exit_layer in enumerate(early_exit_layers):
                ...
                top_indices = np.argsort(hidden_scores_fwd_up[early_exit_layer])[-top_number_ffn:][::-1] if top_number_ffn > 0 else []
                activate_keys_fwd_up[early_exit_layer] = top_indices
                top_indices = np.argsort(hidden_scores_fwd_down[early_exit_layer])[-top_number_ffn:][::-1] if top_number_ffn > 0 else []
                activate_keys_fwd_down[early_exit_layer] = top_indices
                top_indices = np.argsort(hidden_scores_q[early_exit_layer])[-top_number_attn:][::-1] if top_number_attn > 0 else []
                activate_keys_q[early_exit_layer] = top_indices
                top_indices = np.argsort(hidden_scores_k[early_exit_layer])[-top_number_attn:][::-1] if top_number_attn > 0 else []
                activate_keys_k[early_exit_layer] = top_indices
                top_indices = np.argsort(hidden_scores_v[early_exit_layer])[-top_number_attn:][::-1] if top_number_attn > 0 else []
                activate_keys_v[early_exit_layer] = top_indices
                top_indices = np.argsort(hidden_scores_o[early_exit_layer])[-top_number_attn:][::-1] if top_number_attn > 0 else []
